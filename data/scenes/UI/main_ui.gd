...
func show_hide_main_ui(is_ui_visible: bool) -> void:
    ...
    match active_window:
        null:
            if !is_ui_visible:
                for member: BasePlayer in GameManager.player_party:
                    if member.stats.equipment_changed.is_connected(stats_menu.on_equipment_changed):
                        print("equipment_changed already connected!")
                        continue
            
                    member.stats.equipment_changed.connect(stats_menu.on_equipment_changed)
